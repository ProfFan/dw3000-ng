...
use dw3000_ng::{hl, time::Instant, Config};
use nb::block;
...
fn main() -> ! {
    ...
    loop {
        ...
        let t2: u64 = block!(receiving.r_wait(&mut buffer))
            .expect("error during the reception")
            .rx_time
            .value();
        dw3000 = receiving
            .finish_receiving()
        ...
        defmt::println!(
            "Distance = {} m",
            calc_distance_double(t1, t2, t3, t4, t5, t6)
        );
